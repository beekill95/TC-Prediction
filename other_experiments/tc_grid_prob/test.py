...
# +
exp_name = 'tc_grid_prob_unet_iou'
...
@hard_negative_mining
def hard_negative_mined_KL_divergence_loss(y_true, y_pred):
    l = tf.keras.losses.KLDivergence(reduction=tf.keras.losses.Reduction.NONE)
    return l(y_true, y_pred)
...
model.compile(
    optimizer='adam',
    # loss=hard_negative_mined_KL_divergence_loss,
    # loss=dice_loss,
...
def plot_groundtruth_and_prediction(tc_df):
    iou = BBoxesIoUMetric(iou_threshold=0.2)
    ...
    for _, row in tc_df.iterrows():
        ...
        iou.update_state([tf.cast(groundtruth, dtype=tf.float32)], [tf.cast(prediction, dtype=tf.int64)])
...
test_with_tc_df = test_df[test_df['TC']].sample(15)
plot_groundtruth_and_prediction(test_with_tc_df)

# ## Without TC

test_without_tc_df = test_df[~test_df['TC']].sample(15)
