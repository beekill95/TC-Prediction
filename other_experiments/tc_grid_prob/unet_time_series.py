...
input_layer = keras.Input(data_shape[:2] + (data_shape[-1]*2,))
...
training = data_loader.load_dataset(
    train_path,
    batch_size=64,
    shuffle=True).map(subtract)
validation = data_loader.load_dataset(val_path).map(subtract)
