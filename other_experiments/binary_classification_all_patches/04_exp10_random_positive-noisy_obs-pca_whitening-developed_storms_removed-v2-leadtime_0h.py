...
scaler = load_pickle('scaler_developed_storms_removed_v2_pca_whitening.pkl')
pca = load_pickle('pca_developed_storms_removed_v2_pca_whitening.pkl')
...
for X, _ in tqdm(iter(train_patches_ds)):
    emb = preprocessing(X)
    embeddings.append(emb)

embeddings = np.concatenate(embeddings, axis=0).reshape((-1, 30))
tsne = TSNE()
x = tsne.fit_transform(embeddings[:10000])
...
model.save(f'saved_models/binary_classification_all_patches/04_exp10_{metrics[-1]:.3f}')
