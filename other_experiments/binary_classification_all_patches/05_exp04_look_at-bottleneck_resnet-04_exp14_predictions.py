...
plot_patch(false_positives.iloc[80], 'False Positive')

plot_patch(false_positives.iloc[100], 'False Positive')
...
def find_genesis_location(pred_df: pd.DataFrame, ibtracs_df: pd.DataFrame, domain_size=30):
    ...
    ibtracs_df = ibtracs_df[ibtracs_df['BASIN'].isin(['WP', 'EP'])]
    ibtracs_df['Date_storm_leadtime'] = ibtracs_df['Date_storm'].apply(lambda d: d - timedelta(hours=0))
    # Cheated a bit.
