...
model.fit(
    ...
    callbacks=[
        ...
        keras.callbacks.ModelCheckpoint(
            f'saved_models/binary_classification_all_patches/04_exp14_{date_str}_last_weights/weights',
            monitor='loss',
            mode='min',
            save_best_only=True,
...
model.save(f'saved_models/binary_classification_all_patches/04_exp14_{date_str}_{metrics[-1]:.3f}')
...
# +
model.load_weights(f'saved_models/binary_classification_all_patches/04_exp14_{date_str}_last_weights/weights')
feature_map = keras.Model(
    inputs=model.inputs,
    outputs=model.get_layer(name='flatten').output,
)
