...
def MyModel():
    ...
    x = layers.MultiHeadAttention(num_heads=6, key_dim=512, dropout=0.5)(x, x)
    x = keras.Sequential([
        layers.Flatten(),
        layers.Dropout(0.5),
...
model.save(f'saved_models/random_positive_no_pca_multihead_att_developed_storms_removed_f1_{metrics[-1]:.3f}')
