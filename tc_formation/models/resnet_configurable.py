...
def _stack0v2(x, filters, blocks, kernel_size=3, stride1=2, name=None):
    """A set of stacked residual blocks.
    ...
    Returns:
      Output tensor for the stacked blocks.
    """
    x = _block0v2(x, filters, kernel_size=kernel_size, stride=stride1, name=name + '_block1')
    for i in range(2, blocks + 1):
        x = _block0v2(x, filters, kernel_size=kernel_size, conv_shortcut=False,
                    name=name + '_block' + str(i))
    return x
...
def ConfigurableResNet18v2(include_top=True,
             input_tensor=None,
             input_shape=None,
             pooling=None,
             classes=1000,
    ...
    def stack_fn(x):
        x = _stack0v2(x, starting_channels, 2, kernel_size=kernel_size, stride1=1, name='conv2')
        x = _stack0v2(x, starting_channels*2, 2, kernel_size=kernel_size, name='conv3')
        x = _stack0v2(x, starting_channels*4, 2, kernel_size=kernel_size, name='conv4')
        return _stack0v2(x, starting_channels*8, 2, kernel_size=kernel_size, name='conv5')
